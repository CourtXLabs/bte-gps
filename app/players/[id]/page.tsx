import PlayerDashboardToolbar from "@/components/sections/customer/player-details/PlayerDashboardToolbar"
import PlayerGamesFilter from "@/components/sections/customer/player-details/PlayerGamesFilter"
import ReportsList from "@/components/sections/customer/player-details/ReportsList"
import ComboTimesUsedChart from "@/components/sections/customer/player-details/charts/ComboTimesUsedChart"
import PointsComboBarChart from "@/components/sections/customer/player-details/charts/PointsComboBarChart"
import SequencesGraphs from "@/components/sections/customer/player-details/charts/SequencesGraphs"
import InsightsButton from "@/components/sections/customer/player-details/insights/InsightsButton"
...
export default async function PlayerDetailsPage({ params, searchParams }: Props) {
  const { games: gamesParam, season: seasonParam } = searchParams
  ...
  const [playersResponse, reportsResponse, comboPointsResponse, dribbleCounts, seasons] = await Promise.all([
    getAllPlayers(),
    getReports(params.id, games, season),
    getComboPointsRatio(params.id, games, season),
    getDribblesCounts(params.id, games, season),
      ...
      <div className="mx-auto flex w-full flex-col gap-10 px-4 py-10">
        <div className="mx-auto flex w-full max-w-7xl flex-wrap justify-between gap-6">
          <div className="space-y-6">
            ...
            <InsightsButton id={params.id} canEdit={isAdmin} />
          </div>
