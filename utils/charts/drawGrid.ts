...
const drawGrid = ({ chart, width, y, tickValues }: Props) => {
  ...
  if (tickValues) {
    ticks.tickValues(tickValues)
  }

  // Grid lines
  chart.append("g").attr("class", "grid").call(ticks).style("color", "#E7EAEE").style("opacity", 0.35)
}
